...
files_list = sorted(glob.glob('../../data/hide/hide_sims_train/calib_1year/*.fits')) #test_files = sorted(glob.glob('../../data/hide/hide_sims_test/calib_1month/*.fits'))
test_files = sorted(glob.glob('../../data/hide/hide_sims_valid/*.fits'))

the_print('number of training files: '+str(len(files_list)))
#dpt = DataProvider(nx=ws,a_min=0, a_max=200, files=files_list, n_class=1)
if ntry=='1':
    nnn = 5
#elif ntry=='2':
else:
...
print(name)
model_add = './models/hide_model_'+name
pred_dir = 'predictions/hide_model_'+name+'/'
ch_mkdir(pred_dir)
res_file = 'results/hide_model_'+name
ch_mkdir('results')
