...
#        self.dirrty_list = sorted(glob(prefix+'dirty_abs_*.h5'))
        ...
        self.files_list = glob(dataset_add+'*.fits')
        
        self.n_files = len(self.files_list)
        self.inds = np.arange(self.n_files)
    ...
    def reload(self,inds):
        ...
        Y = []
        for i in indsr:      
            f = fits.open(self.files_list[i])
            fp = f[1].data
            data,rfi = fp['DATA'].squeeze().T,fp['RFI'].squeeze().T

            if data.shape != (276, 14400) or rfi.shape!=(276, 14400):
                print(data.shape,rfi.shape)
                print(self.files_list[i])
                continue
...
#        X,Y = np.array(X),np.array(Y)
    ...
    def get_data(self,n):
        if self.n_call % self.reload_freq==0:
            self.X_train,self.Y_train = self.reload(self.inds)
            self.n_train = len(self.X_train)
#            print(self.X_train.shape,self.Y_train.shape)
        t0 = time()
